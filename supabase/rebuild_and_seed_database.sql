...
-- Complete Master Database Drop, Rebuild & Fresh SaaS Production Seed Script
-- ==============================================================================
-- INSTRUCTIONS:
-- 1. Open your Supabase Dashboard -> SQL Editor
-- 2. Paste this ENTIRE file into a New Query
-- 3. Click "RUN"
-- This will wipe old structures, rebuild all 15 tables with indexes, RLS, triggers,
-- and seed fresh production-ready demo data.
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- 1. DROP EXISTING TABLES, VIEWS, TRIGGERS & FUNCTIONS (CLEAN SLATE)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 3. SCHEMA DEFINITION (15 SAAS TABLES)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. PERFORMANCE B-TREE INDEXES (<50ms Target Budget)
-- ------------------------------------------------------------------------------
-- Stores & Profiles
...
-- ------------------------------------------------------------------------------
-- 5. DATABASE BUSINESS LOGIC TRIGGERS (INVENTORY & LEDGER REBALANCING)
-- ------------------------------------------------------------------------------
...
-- Allow public read access to active store identity for login screen verification
...
CREATE POLICY "Profiles self and tenant access" ON public.profiles
    ...
    USING (
        ...
        OR phone IS NOT NULL -- allow auth lookup by phone
    )
    WITH CHECK (
        id = auth.uid()
        OR store_id = ((auth.jwt() -> 'app_metadata' ->> 'store_id')::uuid)
...
-- Tenant Isolation RLS on all operational data tables
DO $$
DECLARE
    t text;
    tables text[] := ARRAY[
    ...
    FOREACH t IN ARRAY tables LOOP
        EXECUTE format('
            ...
            USING (
                ...
                OR (auth.jwt() ->> ''role'') = ''super_admin''
                OR (auth.jwt() -> ''app_metadata'' ->> ''role'') = ''super_admin''
                OR store_id = ''a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11''::uuid -- Demo Flagship Store
            )
            WITH CHECK (
                store_id = ((auth.jwt() -> ''app_metadata'' ->> ''store_id'')::uuid)
                OR (auth.jwt() ->> ''role'') = ''super_admin''
...
-- 2. Staff Profiles (Salted SHA-256 Hashes matching app default credentials)
INSERT INTO public.profiles (
    id, store_id, full_name, phone, role, password_hash, pin_hash, is_active
) VALUES
-- Platform Super Admin (Phone: 01700000000 | Password: admin123 | PIN: 1234)
(
    'f0000000-0000-0000-0000-000000000000',
    'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
    'সুপার অ্যাডমিন (System Administrator)',
    '01700000000',
    'super_admin',
    'sha256$4b187707c7394e03030383e54ab9c7bd628b340134810d55207cadeb53c50499',
    'sha256$4b187707c7394e03030383e54ab9c7bd628b340134810d55207cadeb53c50499',
    TRUE
),
-- Shop Owner (Phone: 01711998877 | Password: dokan123 | PIN: 1234)
(
    'f0000001-0000-0000-0000-000000000001',
    'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
    'মোঃ রফিকুল ইসলাম (দোকান মালিক)',
    '01711998877',
    'owner',
    'sha256$9b4198a88a6c92cb3d1d566d46dec30f8407c71f20a0fd6d9801db5c44ad1ef7',
    'sha256$9b4198a88a6c92cb3d1d566d46dec30f8407c71f20a0fd6d9801db5c44ad1ef7',
    TRUE
),
-- Manager (Phone: 01811223344 | Password: dokan123 | PIN: 1234)
(
    'f0000002-0000-0000-0000-000000000002',
    'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
    'আব্দুল করিম (ম্যানেজার)',
    '01811223344',
    'manager',
    'sha256$d41b835cce5873cc227dc8c7805639488a4315199accfdd0efed99ef11b07379',
    'sha256$d41b835cce5873cc227dc8c7805639488a4315199accfdd0efed99ef11b07379',
    TRUE
),
-- Cashier (Phone: 01911334455 | Password: dokan123 | PIN: 1234)
(
    'f0000003-0000-0000-0000-000000000003',
    'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
    'তানভীর হাসান (ক্যাশিয়ার)',
    '01911334455',
    'cashier',
    'sha256$3a4b261157f67591802d9a941520105ed0444ab404fd084cd6f29ea93b90ce53',
    'sha256$3a4b261157f67591802d9a941520105ed0444ab404fd084cd6f29ea93b90ce53',
    TRUE
);

-- 3. Standard Grocery Categories
...
-- 4. Initial Catalog Products (Fast Items + Packaged Barcoded Items)
...
-- Fast unbarcoded items for POS touch screen grid
...
-- 5. Regular Customers (Baki Khata)
...
-- 6. Sample Initial Ledger Transactions (Triggers will compute the initial balances)
...
-- ==============================================================================
-- ✅ DATABASE REBUILD & FRESH SAAS SEED COMPLETE!
-- ==============================================================================
